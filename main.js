...
const readfile = JSON.parse(
  decoder.decode(Deno.readFileSync("./supersecret.txt")),
);
try {
...
disconn.on(Events.MessageCreate, async (message) => {
  ...
  if (
    message.channelId === readfile.DISCORDCHANNELID &&
    conn.readyState === WebSocket.OPEN
  ) {
    let messageContent = `${message.author.username}: ${
      message.content.replace(
        /<:.*?:(\d+)>/g,
        "![](https://cdn.discordapp.com/emojis/$1.png?size=16)",
      )
    }`;
    ...
    /* conn.send(
      JSON.stringify({ "cmd": "post", "p": messageContent, "token": MAETOKEN }),
    );
    */
    let dbData = [];
    try {
    ...
    const userEntry = dbData.find((entry) =>
...
disconn.on(Events.InteractionCreate, async (interaction) => {
  ...
  switch (interaction.commandName) {
    ...
    case "echo":
      ...
      break;
    case "albuquerque":
      const albuquerqueMessage = decoder.decode(
        Deno.readFileSync("./albuquerque.txt"),
      );
      await interaction.reply(albuquerqueMessage);
      break;
    case "miaugif":
      await interaction.reply(
        "https://tenor.com/view/miau-hd-adobe-after-effects-glass-breaking-preset-gif-752576862881430143",
      );
      /*
            break;
        case 'toggle_react':
